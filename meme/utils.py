...
class MediaResolver:
    ...
    async def download_media(self, url: str, target_path: str, event: AstrMessageEvent):
        ...
        if url.startswith("file://"):
            ...
            shared_path = self._map_napcat_container_path(file_path)
            if shared_path and os.path.isfile(shared_path):
                shutil.copy(shared_path, target_path)
                return
            ...
            raise Exception(
                f"找不到本地文件: {file_path}。"
                "若 NapCat 与 AstrBot 异机部署，请配置 napcat_media_share_path "
                "和 napcat_media_container_path。"
            )
        ...
        shared_path = self._map_napcat_container_path(unquoted)
        if shared_path and os.path.isfile(shared_path):
            shutil.copy(shared_path, target_path)
            return
    ...
    def _map_napcat_container_path(self, path: str):
        """Map a NapCat container path to its Windows-accessible shared directory."""
        share_dir = str(self.config.get("napcat_media_share_path", "")).strip().strip('"').strip("'")
        container_dir = str(self.config.get("napcat_media_container_path", "")).strip().strip('"').strip("'")
        if not share_dir or not container_dir or not path:
            return None

        normalized_path = str(path).replace("\\", "/")
        normalized_root = container_dir.replace("\\", "/").rstrip("/")
        try:
            relative_path = PurePosixPath(normalized_path).relative_to(PurePosixPath(normalized_root))
        except ValueError:
            return None

        if any(part in ("", ".", "..") for part in relative_path.parts):
            return None
        return os.path.join(share_dir, *relative_path.parts)
